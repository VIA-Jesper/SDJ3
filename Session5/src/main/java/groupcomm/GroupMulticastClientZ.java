...
public class GroupMulticastClientZ
{
  ...
  public static void main(String[] args) throws UnknownHostException
  {
    ...
    try (MulticastSocket clientSocketZ = new MulticastSocket(PORT)) {
      // join the group
      clientSocketZ.joinGroup(address);
      // Recieve the group message
      while (true) {
        // create a packet with data
        DatagramPacket msgPacket = new DatagramPacket(buf, buf.length);
        // receive the packet
        clientSocketZ.receive(msgPacket);
        String msgReceived = new String(buf, 0, buf.length);
        System.out.println("Z group recieved: " + msgReceived);
      }
